...
const Shop: React.FC = () => {
  ...
  }, [category, brand, priceRange, sort, products]);
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        <aside className={`${showFilters ? 'block' : 'hidden'} lg:block w-full lg:w-64 flex-shrink-0 space-y-10 bg-white lg:bg-transparent p-6 lg:p-0 rounded-3xl lg:rounded-none`}>
          <div className="space-y-4">
            <h3 className="font-bold text-gray-900 flex items-center justify-between">Categories<ChevronDown className="h-4 w-4" /></h3>
          ...
          <div className="space-y-4">
            ...
            <input type="range" min="0" max="10000" step="500" className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-rose-500" value={priceRange} onChange={e => setPriceRange(Number(e.target.value))} />
            <div className="flex justify-between text-xs font-bold text-gray-500"><span>৳0</span><span>Max: ৳{priceRange}</span></div>
          </div>
          <div className="space-y-4">
            ...
            {filteredProducts.map(product => (
              <div key={product.id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-gray-100 p-3">
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden mb-4">
                  ...
                  {product.salePrice && <span className="absolute top-3 left-3 bg-rose-500 text-white text-[10px] font-bold px-2 py-1 rounded">SALE</span>}
                </div>
